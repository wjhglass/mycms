			...
			<?php if (isset($this->vars['levels']) && $this->vars['levels'] == true) {?>
				<?php foreach ($this->vars['levels'] as $key=>$value) { ?>
					<tr>
						...
						<td><a href="level.php?action=edit&id=<?php echo $value->id?>">编辑</a> | <a href="content.php?action=delete&id=<?php echo $value->id?>" onclick="return confirm('您真的要删除吗？');">删除</a></td>
					</tr>
				<?php } ?>
			<?php } else { ?>
	...
	<?php if (isset($this->vars['add']) && $this->vars['add'] == true) {?>
		<form method="post" name="add">
			<table cellspacing="0" class="content">
				...
				<tr><td>缩  略  图：<input type="text" name="thumbnail" class="text" /><input type="button" value="上传缩略图" /></td></tr>
